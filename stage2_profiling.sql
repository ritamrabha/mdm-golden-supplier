-- ============================================================
-- STAGE 2: PROFILING
-- Quantifies exactly what's wrong with the raw data before any
-- standardization or matching happens. Results are persisted to
-- STAGING.profile_results, not just eyeballed in a worksheet —
-- so the profile is reproducible and diffable across runs.
-- ============================================================

USE DATABASE mdm_golden_supplier;

-- 1. Create the profiling results table
...
-- 3. Null counts + percentage on business-critical fields
INSERT INTO STAGING.profile_results (source_table, column_name, metric_name, metric_value)
...
-- 4. Distinct name count (cardinality check before matching)
INSERT INTO STAGING.profile_results (source_table, column_name, metric_name, metric_value)
...
-- 5. Exact duplicate names (same name string appears more than once)
INSERT INTO STAGING.profile_results (source_table, column_name, metric_name, metric_value)
...
-- 6. Irregular whitespace (leading/trailing/double spaces) — this is the
--    corruption we injected on purpose, and it's exactly what standardization
--    in Stage 3 needs to fix.
INSERT INTO STAGING.profile_results (source_table, column_name, metric_name, metric_value)
...
-- 8. Freshness range (oldest / newest last_updated)
INSERT INTO STAGING.profile_results (source_table, column_name, metric_name, metric_value)
...
-- 9. Review everything that was found
